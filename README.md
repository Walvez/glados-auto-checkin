...
适合没有常开电脑/手机代理的场景。工作流使用 Node.js CLI，复用与本仓库一致的响应校验、有限重试与同域名签到逻辑；签到 API 的 body token 为 `glados.cloud`（适配 2026 年接口变化）。
...
- GitHub Actions / CLI 使用 Cookie Secret，按 `glados.cloud` 优先的顺序探测 6 个域名，在**同一已登录域名**上完成签到；body token 为 `glados.cloud`。
...
<details>
<summary><strong>为何返回 please checkin via https://glados.cloud</strong></summary>

这是 2026 年接口变更的典型现象：签到请求 body 中的 token 需为 `glados.cloud`。本仓库 CLI 与脚本已使用该 token。若你使用其它旧脚本仍带 `glados.one`，请改用本仓库或自行更新 token。
