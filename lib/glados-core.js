 ...
 *
 * Check-in body token uses `glados.cloud` (2026 API). Older `glados.one`
 * tokens may return "please checkin via https://glados.cloud".
 */
...
function classifyCheckin(result) {
  ...
  // Reject redirect-style failures such as "please checkin via https://glados.cloud"
  if (/please checkin via/i.test(detail)) {
    throw new Error(`签到接口返回异常：${detail}（请确认 check-in token 为 ${CHECKIN_TOKEN}）`);
  }
 ...
 *   2. JSON array of objects: [{"cookie":"...","name":"work"}, ...]
...
function normalizeAccountEntry(entry, index) {
  ...
  if (typeof entry === "string") {
    ...
    return { name: label, cookie, authorization: "" };
  }
  ...
  return { name, cookie, authorization };
}
